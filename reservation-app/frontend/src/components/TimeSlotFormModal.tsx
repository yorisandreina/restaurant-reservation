...
const defaultForm: DaySlot[] = daysOfWeek.map((day) => ({
  dow: day.dow,
  closed: false,
  startTime: "00:00",
  endTime: "00:00",
...
const validateDaySlot = (day: DaySlot): string | null => {
  ...
  if (!day.slotMin || day.slotMin <= 0) {
    return "El intervalo del slot debe ser mayor que 0.";
  }

  if (!day.maxDuration || day.maxDuration <= 0) {
    return "La duración máxima debe ser mayor que 0.";
  }
  ...
  if (openMinutes < day.maxDuration) {
    return "La franja horaria debe ser igual o mayor que la duración máxima.";
  }

  if (day.slotMin > day.maxDuration) {
    return "El slot no debería ser mayor que la duración máxima.";
  }
...
const TimeSlotFormModal: React.FC<TimeSlotFormModalProps> = ({
  ...
  const [form, setForm] = React.useState<DaySlot[]>(defaultForm);
  ...
  const handleChange = (
    ...
    setForm((prev) => {
      ...
      next[index] = {
        ...next[index],
        [field]:
          field === "slotMin" || field === "maxDuration"
            ? Number(value)
            : value,
      };
      return next;
  ...
  const handleBlurDay = (index: number) => {
    setTouched((prev) => ({
      ...prev,
      [index]: true,
    }));
  ...
  const dayErrors = form.map((day) => validateDaySlot(day));
  const hasErrors = dayErrors.some((err) => err !== null);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex flex-col gap-4 overflow-y-auto pr-1 max-h-[70vh]">
          {form.map((day, index) => {
            const showDayError = touched[index] && dayErrors[index];
              ...
              >
                ...
                {!day.closed && (
                  <>
                    ...
                    {showDayError && (
                      <p className="text-sm text-red-500">{dayErrors[index]}</p>
                    )}
                  </>
                )}
              </div>
        ...
        <DialogFooter className="mt-4 flex justify-end gap-2">
          ...
          <Button onClick={handleSubmit} disabled={loading || hasErrors}>
